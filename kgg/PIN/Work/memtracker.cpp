...
VOID dataman (THREADID tid, ADDRINT ina, ADDRINT memOp1, ADDRINT memOp2, ADDRINT memOp3)
{
	...
	std::cout << "DATAMAN DEWANG \n";
    PIN_LockClient();
    IMG image = IMG_FindByAddress(ina);
    PIN_UnlockClient();

    if(IMG_Valid(image)==false)
        return;

    ADDRINT baseaddr = IMG_LowAddress(image);
    std::cout<<"BASEADDR :: "<<std::hex<<baseaddr<<std::dec<<"\n";
	...
	if (memOp1 != -1)
	{
	    std::cout<<"ADDR :: "<<std::hex<<memOp1-baseaddr<<std::dec<<"\n";
	...
	if (memOp2 != -1)
	{
	    std::cout<<"ADDR :: "<<std::hex<<memOp2-baseaddr<<std::dec<<"\n";
	...
	if (memOp3 != -1)
	{
	    std::cout<<"ADDR :: "<<std::hex<<memOp3-baseaddr<<std::dec<<"\n";
...
VOID ImageLoad(IMG img, VOID *v)
{
	...
	ADDRINT img_loff = IMG_LowAddress(img);
	...
	while (addrFile.good())
	{
		...
		addrFile >> address >> dec >> var.name >> var.id;
		addrFile.get();
		// VARTYPE may contain spaces like `struct * ABC [2]`. So get until \t
		...
		std::cout<<"loff "<<std::hex<<img_loff<<std::dec<<" "<<address<<"\n";
		if (var.name != "" && var.spattr=="GLOBAL")
			globalMap[img_loff+strtoul(address.c_str(), NULL, 16)] = var;
	}
